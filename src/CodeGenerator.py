...
class FolderQRProcessor(FileQRProcessor):
            ...
            for entry in os.scandir(folder_path):
                try:
                    ...
                    if entry.is_file():
                        ...
                        # Read file content (similar to _get_file_content method)
                        try:
    ...
    def reconstruct_folder_from_qr(
        ...
        # Reconstruct files and subfolders
        def _reconstruct_entry(entry, current_path):
            if entry["type"] == "file":
                ...
                for child in entry.get("children", []):
                    _reconstruct_entry(child, subdir_path)

        # Start reconstruction
        for child in folder_structure.get("children", []):
            _reconstruct_entry(child, full_output_path)
